    ...
    <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg p-8 ring-1 ring-zinc-900/5">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          />
          {errors.name && (
            <p className="mt-2text-sm text-red-600">{errors.name}</p>
          )}
        </div>
        <div>
          <label htmlFor="" className="block text-sm font-medium text-zinc-700">
        ...
        <div>
          ...
          <select
            ...
            id=""
            onChange={handleChange}
            className={`mt-1block w-full rounded-lg border ${
              errors.role
                ? "border-red-300 ring-red-500"
                : "border-zinc-300 ring-blue-500"
            } px-3 py-2 text-sm focus:outline-none focus:ring-2`}
        ...
        <div>
          <h2 className="block text-sm font-medium text-zinc-700 mb-2">
            Skills
          </h2>
          <div className="grid grid-cols-2 gap-4">
            {skillChoices.map((choice, index) => (
              <label key={index} className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  checked={formData.skills.includes(choice)}
                  onChange={() => handleSkillChange(choice)}
                  className="h-4 w-4 rounded border-zinc-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm text-zinc-600">{choice}</span>
              </label>
            ))}
          </div>
          {errors.skills && (
        ...
        <div className="flex items-center space-x-2">
          <input
            ...
            className="h-4 w-4 rounded border-zinc-300 text-blue-600 focus:ring-blue-500"
          />
          <label htmlFor="" className="text-sm text-zinc-700">
        ...
        <button
          type="submit"
          className="w-full rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold
             text-white shadow-sm hover:bg-rose-500 focus:outline-none focus:ring-2
